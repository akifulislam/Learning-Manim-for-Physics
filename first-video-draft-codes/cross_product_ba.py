...
class VectorCrossProductBA(ThreeDScene):
    def construct(self):
        ...
        vector_D_coord = -np.cross(vector_A_coord, vector_B_coord)  # Cross product
        ...
        unit_vector_eta = vector_D_coord / np.linalg.norm(vector_D_coord)
        ...
        label_C = always_redraw(lambda: MathTex(r"\vec{C}", color=TEAL_D).rotate(PI/2, axis=RIGHT).next_to(vector_D.get_end(), RIGHT + UP, buff=0.1))
        ...
        elements_group = VGroup(axes, x_label, y_label, z_label, vector_A, vector_B, vector_eta, vector_D, parallelogram, label_A, label_B, label_C, label_eta)
        
        # Display the vectors, labels, and parallelogram
        self.play(Create(elements_group), rate_func=smooth, run_time=1.5)
        ...
        cross_product_text = MathTex(
            r"\vec{B} \times \vec{A} = \hat{\eta} \, | \vec{B} | \, | \vec{A} | \, \sin \theta_{BA} = \vec{C}"
        ).set_color_by_gradient(RED, BLUE)

        length_text = MathTex(
            r"| \vec{D} | = | \vec{B} | \, | \vec{A} | \, \sin \theta_{BA} = D_{BA}"
        ...
        where1_text = MathTex(
            r"\theta_{BA} \text{ is the angle between } \vec{B} \text{ and } \vec{A};"
        ).set_color_by_gradient(RED, BLUE)

        where2_text = MathTex(
            r"\hat{\eta} \text{ is the unit vector perpendicular to both } \vec{B} \text{ and } \vec{A};"
        ).set_color_by_gradient(LIGHT_BROWN, BLUE)

        where3_text = MathTex(
            r"C_{BA} \text{ is the area of the parallelogram formed by } \vec{B} \text{ and } \vec{A}."
        ...
        # Rotate the graph with camera movements
        #       self.move_camera(phi=75 * DEGREES, theta=-90 * DEGREES, run_time=1)
        self.wait(2)
        ...
        self.play(
            ...
            Unwrite(vector_A), Unwrite(vector_B), Unwrite(vector_D), Unwrite(vector_eta),
        ...
        vectors_group = VGroup(vector_A, vector_B).arrange(DOWN, buff=0.25)
        
        # Display the 3x3 determinant setup
        determinant = MathTex(
            r"\vec{B} \times \vec{A} = \begin{vmatrix}"
        ...
        cross_product = MathTex(
            r"\vec{B} \times \vec{A} = ",
            r"\hat{i}(", r"B_y", r"A_z", r"-", r"B_z", r"A_y", r") - ",
            r"\hat{j}(", r"B_x", r"A_z", r"-", r"B_z", r"A_x", r") + ",
            r"\hat{k}(", r"B_x", r"A_y", r"-", r"B_y", r"A_x", r")"
        ).scale(1.25).move_to(RIGHT * 0.0 + UP * -1.5).set_color_by_tex_to_color_map({
            "B_x": B_x_color, "B_y": B_y_color, "B_z": B_z_color,
            "A_x": A_x_color, "A_y": A_y_color, "A_z": A_z_color,
            r"\hat{i}": i_color, r"\hat{j}": j_color, r"\hat{k}": k_color
        })
